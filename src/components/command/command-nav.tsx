...
/**
 * Textura-edition top navigation for the full-bleed Command Center.
 *
 ...
 * compromise that keeps it both faithful to Textura and fully usable: a slim
    ...
    <>
      ...
      >
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-10">
          <Link href="/intro" className="tx-display text-lg tracking-tight text-warm-strong">
            ...
            {CORE.map((n) => {
              ...
              return (
                <Link
                  ...
                  href={n.href}
                  className={cn(
                    "tx-eyebrow transition-colors hover:text-warm-strong",
                    active ? "text-[var(--color-tx-accent)]" : "text-warm/65"
                  )}
                >
                  {n.label}
                </Link>
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/verify"
              className="tx-cta hidden items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold sm:inline-flex"
            >
            ...
            <button
              ...
              className="tx-ghost inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium"
            >
        ...
        <div className="cc-menu fixed inset-0 z-[60] overflow-y-auto">
          <div className="mx-auto flex min-h-screen max-w-7xl flex-col px-5 py-4 lg:px-10">
            <div className="flex items-center justify-between py-1">
              <span className="tx-display text-lg text-warm-strong">
                VITAL<span className="text-[var(--color-tx-accent)]">CHAIN</span>
              </span>
              <button
            ...
            <p className="tx-eyebrow mt-10 text-[var(--color-tx-accent)]">Modules</p>
            ...
            <p className="tx-eyebrow mt-12 text-[var(--color-tx-accent)]">Tools</p>
            <div className="mt-5 grid gap-x-10 sm:grid-cols-2">
            ...
            <footer className="mt-auto flex flex-col gap-2 border-t border-white/10 pt-8 pb-2 text-sm text-warm/40 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="tx-eyebrow">CDSCO · India · multi-market</span>
            </footer>
          </div>
        </div>
      )}
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active ? "text-[var(--color-tx-accent)]" : "text-warm-strong"
      )}
    >
      <span className="tx-display text-2xl transition-transform duration-300 group-hover:translate-x-1.5 lg:text-3xl">
        {item.label}
      </span>
      <ArrowUpRight
        size={22}
        className="shrink-0 text-warm/30 transition-colors group-hover:text-[var(--color-tx-accent)]"
      />
    </Link>
  );
}
